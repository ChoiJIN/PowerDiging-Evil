...
void GLDraw::planeWithTexture(Vector3 pts[4], int textureId)
{
	glBegin(GL_QUADS);
	for (int i = 0; i < 4; i++)
		glVertex3d(pts[i].x, pts[i].y, pts[i].z);
...
void GLDraw::room(float size, int textureId[6])
{
	Vector3 pts[8];
	...
	// side
	glColor3d(1, 1, 1);
	plane(pts[0], pts[3], pts[5], pts[6]);
	glColor3d(1, 0, 0);
	plane(pts[0], pts[1], pts[2], pts[3]);
	glColor3d(1, 1, 1);
	plane(pts[7], pts[4], pts[2], pts[1]);
	glColor3d(1, 1, 0);
	plane(pts[6], pts[5], pts[4], pts[7]);

	// bottom
	glColor3d(0, 1, 0);
	plane(pts[2], pts[4], pts[5], pts[3]);

	// top
	glColor3d(0, 0, 1);
	plane(pts[7], pts[1], pts[0], pts[6]);
}